...
def test_flash_rejects_exploration_before_services(monkeypatch, capsys, dashboard):
    cli = _cli_module()
    from rpent.robots import get_robot_spec

    def unexpected(*args, **kwargs):
        pytest.fail("services or memory sync started for invalid Flash Mode")

    monkeypatch.setattr("rpent.memory.MemoryManager.sync", unexpected)
    monkeypatch.setattr(cli, "get_toolkit", unexpected)
    spec = get_robot_spec("libero")
    from dataclasses import replace

    monkeypatch.setattr(
        cli, "get_robot_spec", lambda name: replace(spec, init_runtime=unexpected)
    )
    argv = [
        "rpent",
        "--robot",
...
def test_flash_entrypoints_sync_only_hf_plans(
    monkeypatch, tmp_path, dashboard, profile
):
    cli = _cli_module()
    from rpent.dashboard import server, session
    from rpent.robots import get_robot_spec

    root = tmp_path / "memory" / "libero"
    flash = root / "flash"
    flash.mkdir(parents=True)
    (flash / "spatial_task_t0_plan.json").write_text('{"plan": []}')
    (flash / "spatial_task_t0_anchors.json").write_text('{"anchors": []}')
    sync_calls = []
    monkeypatch.setattr(
        "rpent.memory.MemoryManager.sync",
        lambda self, **kwargs: sync_calls.append((self.root, kwargs)),
    )
    monkeypatch.setattr(cli, "get_memory_dir", lambda robot: root)
    monkeypatch.setattr("rpent.cli.dashboard.get_memory_dir", lambda robot: root)
    monkeypatch.setattr(server.DashboardServer, "start", lambda self: "http://offline")

    def stop(*args, **kwargs):
        raise ConfigCaptured

    monkeypatch.setattr(cli, "build_planner", stop)
    monkeypatch.setattr(session, "DashboardSessionController", stop)
    argv = [
        "rpent",
        "--robot",
        "libero",
    ...
    if profile == "local":
        argv.extend(["--memory-dir", str(root)])
    if dashboard:
        argv.append("--dashboard")
    monkeypatch.setattr(sys, "argv", argv)
    with pytest.raises(ConfigCaptured):
    ...
    expected = (
        [
            (
                root,
                {
                    "remote_repo": get_robot_spec("libero").memory_repo_id,
                    "allow_patterns": ("libero/flash/**",),
                },
            )
        ]
        if profile == "hf"
